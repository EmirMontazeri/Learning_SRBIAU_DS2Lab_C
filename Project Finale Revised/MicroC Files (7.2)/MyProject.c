...
void StartSignal(){
 TRISB.B0 = 0; //Configure RD0 as output
 PORTB.B0 = 0; //RD0 sends 0 to the sensor
 delay_ms(18);
 PORTB.B0 = 1; //RD0 sends 1 to the sensor
 delay_us(30);
 TRISB.B0 = 1; //Configure RD0 as input
 }
 //////////////////////////////
 void CheckResponse(){
 Check = 0;
 ...
 char i, j;
 for(j = 0; j < 8; j++){
 while(!PORTB.B0); //Wait until PORTD.F0 goes HIGH
 delay_us(30);
 if(PORTB.B0 == 0)
 i&= ~(1<<(7 - j)); //Clear bit (7-b)
 else {i|= (1 << (7 - j)); //Set bit (7-b)
 while(PORTB.B0);} //Wait until PORTD.F0 goes LOW
 }
 return i;
 }
 //////////////////////////////
